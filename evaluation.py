...
def evaluationBypandas(df):
    mrr= df.groupby("qid").apply(mrr_metric).mean()
    map= df.groupby("qid").apply(map_metric).mean()
    percsisionAT1= df.groupby("qid").apply(percisionAT1_metric).mean()
